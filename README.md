...
Panel template `example/panel.svg` — drawn in Inkscape with two empty placeholder groups `sinus` and `cosinus`:

![panel template](example/panel.svg)

Input figures generated by `example/sinus_cosine.py`:

| `sinus.svg` | `cos.svg` |
|:-----------:|:---------:|
| ![sinus](example/sinus.svg) | ![cos](example/cos.svg) |
...
```bash
cd example && python sinus_cosine.py
pc example/pc.yaml
```
